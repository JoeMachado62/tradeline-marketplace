  ...
  actions: {
    ...
    async loadConfig() {
      try {
        ...
        console.log('[TradelineWidget] Loaded remote config:', remoteConfig);

        // Merge remote config into local state
        // This ensures DB settings (like allow_promo_codes) override local defaults
        if (this.config) {
          // Careful merge to preserve existing config while overriding with remote
          ...
          console.log('[TradelineWidget] Final merged config:', this.config);
        }

        // Apply theme if provided
        if (remoteConfig.theme) {
          this.applyTheme(remoteConfig.theme);
        }
      } catch (error) {
    ...
    async checkoutWithDocuments(data: {
      ...
      try {
        ...
        // Send promo code if present
        if (this.promoCode) formData.append("promo_code", this.promoCode);
